...
function Step6({ navigation, route }) {
  ...
  const [restroomItem, setRestroomItem] = useState([]);
  const { height, width } = useWindowDimensions();

  const restroom = ["Yes, we have to share ", "No, we will use seperate"];

  const tenantsHandler = (text) => {
    setTenant(text);
}
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <Text style={styles.screenTitleName}>
        ...
        <TextInput onChangeText={tenantsHandler} placeholder="Enter number" 
        ...
        <SelectDropdown
           ...
           dropdownStyle={styles.dropDownOption}
	         onSelect={(restroomItem, index) => {
            setRestroomItem(restroomItem)
		          console.log(restroomItem, index)
	         }}
        ...
        disabled={tenants.length < 1 || restroomItem.length < 1}
